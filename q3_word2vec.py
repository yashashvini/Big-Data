...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    xsum = np.sqrt(np.sum(x**2,1))
    x /= np.reshape(xsum,(-1,1))
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    diffout = output - y
    gradPred = np.dot(diffout,outputVectors)
    grad = np.dot(diffout.T,predicted)
    ### END YOUR CODE
